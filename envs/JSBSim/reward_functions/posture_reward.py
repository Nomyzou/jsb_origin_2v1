...
class PostureReward(BaseRewardFunction):
    ...
    def get_reward(self, task, env, agent_id):
        ...
        """
        new_reward = 0
        min_distance_km = float('inf')  # Track minimum distance for scaling
        ...
        # Apply distance-based scaling: scale down to minimum 0.4 when within 6km
        if min_distance_km <= 6.0:
            # Linear scaling from 1.0 at 6km to 0.4 at 0km
            scale_factor = 0.4 + 0.6 * (min_distance_km / 6.0)
            new_reward *= scale_factor
        new_reward = np.clip(new_reward, -1.1, 1.2)
