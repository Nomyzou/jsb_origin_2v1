...
#         # 配置参数
#         self.evasion_distance = getattr(self.config, f'{self.__class__.__name__}_distance', 6.0)  # 6km
#         self.face_to_face_angle_threshold = getattr(self.config, f'{self.__class__.__name__}_angle_threshold', 30.0)  # 30度
#         self.penalty_scale = getattr(self.config, f'{self.__class__.__name__}_penalty_scale', 10.0)  # 惩罚强度
        
#         # 新增奖励参数
#         self.attack_ao_threshold = getattr(self.config, f'{self.__class__.__name__}_attack_ao_threshold', 20.0)  # AO阈值20度
#         self.attack_ta_threshold = getattr(self.config, f'{self.__class__.__name__}_attack_ta_threshold', 45.0)  # TA阈值120度
#         self.attack_reward_scale = getattr(self.config, f'{self.__class__.__name__}_attack_reward_scale', 5.0)  # 攻击奖励强度
        
#         self.reward_item_names = [self.__class__.__name__ + item for item in ['', '_penalty', '_distance_factor', '_angle_factor', '_attack_reward', '_attack_ao_factor', '_attack_ta_factor']]
...
#             # === 原有惩罚机制：面对面飞行 ===
#             is_face_to_face = (AO_deg < self.face_to_face_angle_threshold and 
#                               TA_deg < self.face_to_face_angle_threshold)
            
#             if is_face_to_face and distance_km <= self.evasion_distance:
#                 # 计算角度因子：两个角度都越小（越接近面对面），惩罚越大
#                 ao_factor = max(0, 1.0 - AO_deg / self.face_to_face_angle_threshold)
#                 ta_factor = max(0, 1.0 - TA_deg / self.face_to_face_angle_threshold)
#                 angle_factor = ao_factor * ta_factor
                
#                 # 计算距离因子：距离越近，惩罚越大
#                 distance_factor = max(0, 1.0 - distance_km / self.evasion_distance)
                
#                 # 计算惩罚值
#                 penalty = self.penalty_scale * angle_factor * distance_factor
...
#                     print(f"Agent {agent_id}: PENALTY - AO={AO_deg:.1f}°, TA={TA_deg:.1f}°, Distance={distance_km:.1f}km, "
#                           f"AngleFactor={angle_factor:.2f}, DistanceFactor={distance_factor:.2f}, "
#                           f"Penalty={penalty:.2f}")
            
#             # === 新增奖励机制：AO≤20°且TA≥120° ===
#             is_good_attack_angle = (AO_deg <= self.attack_ao_threshold and 
#                                    TA_deg >= self.attack_ta_threshold)
            
#             if is_good_attack_angle:
#                 # 计算AO因子：AO越小（越接近0°），奖励越大
#                 attack_ao_factor = max(0, 1.0 - AO_deg / self.attack_ao_threshold)
                
#                 # 计算TA因子：TA越大（越接近180°），奖励越大
#                 attack_ta_factor = max(0, (TA_deg - self.attack_ta_threshold) / (180.0 - self.attack_ta_threshold))
                
#                 # 计算距离因子：距离适中时奖励更大
#                 if 2.0 <= distance_km <= 8.0:
#                     attack_distance_factor = 1.0
#                 elif distance_km < 2.0:
#                     attack_distance_factor = 0.5 + 0.5 * (distance_km / 2.0)  # 过近时奖励递减
#                 else:
#                     attack_distance_factor = max(0.3, 1.0 - 0.1 * (distance_km - 8.0))  # 过远时奖励递减
                
#                 # 计算攻击奖励
#                 attack_reward = self.attack_reward_scale * attack_ao_factor * attack_ta_factor * attack_distance_factor
...
#                 # 记录详细信息用于调试
#                 if hasattr(self, '_debug') and self._debug:
#                     print(f"Agent {agent_id}: REWARD - AO={AO_deg:.1f}°, TA={TA_deg:.1f}°, Distance={distance_km:.1f}km, "
#                           f"AOFactor={attack_ao_factor:.2f}, TAFactor={attack_ta_factor:.2f}, "
#                           f"DistanceFactor={attack_distance_factor:.2f}, AttackReward={attack_reward:.2f}")
        
#         return self._process(reward, agent_id, (reward, penalty, attack_reward, attack_ao_factor, attack_ta_factor))
