...
def plot_advantage_analysis():
    ...
    fig.suptitle('5v4 空战 - 各飞机优势函数值变化', fontsize=16, fontweight='bold')
    
    # 我方飞机ID列表 (5架)
    friendly_ids = ['A0100', 'A0200', 'A0300', 'A0400', 'A0500']
    ...
    plot_filename = f"5v4_advantage_analysis_{timestamp}.png"
...
def plot_target_assignment():
    ...
    # 创建图表 - 2x3布局，5架我方飞机
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))
    fig.suptitle('5v4 空战 - 各飞机目标分配情况', fontsize=16, fontweight='bold')
    ...
    for i, friendly_id in enumerate(friendly_ids):
        ...
        if steps:  # 如果有数据才绘图
            ...
            bars = ax.bar(steps, target_numbers, width=0.8, alpha=0.7, 
                         color=['red', 'blue', 'green', 'orange', 'purple'][i], edgecolor='black', linewidth=0.5)
    ...
    # 保存图表
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plot_filename = f"5v4_target_assignment_{timestamp}.png"
    plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
...
def analyze_target_assignment(target_mapping):
    """
    分析目标分配情况，检查每架敌机被分配到的我方飞机列表
    
    Args:
        target_mapping: 目标分配字典 {friendly_id: target_id}
    
    Returns:
        assignment_analysis: 分配分析结果
    """
    try:
        # 统计每架敌机被多少架我方飞机攻击
        enemy_assignment_count = {}
        enemy_to_friendly_mapping = {}
        
        for friendly_id, target_id in target_mapping.items():
            if target_id not in enemy_assignment_count:
                enemy_assignment_count[target_id] = 0
                enemy_to_friendly_mapping[target_id] = []
            
            enemy_assignment_count[target_id] += 1
            enemy_to_friendly_mapping[target_id].append(friendly_id)
        
        # 找出被多架我方飞机攻击的敌机
        multiple_assignment_enemies = {}
        for enemy_id, count in enemy_assignment_count.items():
            if count > 1:
                multiple_assignment_enemies[enemy_id] = {
                    'count': count,
                    'attackers': enemy_to_friendly_mapping[enemy_id]
                }
        
        # 构建分析结果
        assignment_analysis = {
            'enemy_assignment_count': enemy_assignment_count,
            'enemy_to_friendly_mapping': enemy_to_friendly_mapping,
            'multiple_assignment_enemies': multiple_assignment_enemies,
            'total_enemies_targeted': len(enemy_assignment_count),
            'total_friendly_aircraft': len(target_mapping)
        }
        
        return assignment_analysis
        
    except Exception as e:
        logger.error(f"Error analyzing target assignment: {e}")
        return None

def log_target_assignment_analysis(assignment_analysis, step_count):
    """
    记录目标分配分析结果
    ...
    try:
        ...
        logger.info(f"Step {step_count} - 目标分配分析:")
        logger.info(f"  总敌机数量: {assignment_analysis['total_enemies_targeted']}")
        logger.info(f"  总我方飞机数量: {assignment_analysis['total_friendly_aircraft']}")
        
        # 输出每架敌机的攻击者信息
        for enemy_id, attackers in assignment_analysis['enemy_to_friendly_mapping'].items():
            attacker_count = len(attackers)
            if attacker_count > 1:
                logger.info(f"  🎯 {enemy_id} 被 {attacker_count} 架我方飞机攻击: {', '.join(attackers)}")
            else:
                logger.info(f"  {enemy_id} 被 {attacker_count} 架我方飞机攻击: {attackers[0]}")
        
        # 特别标注被多架飞机攻击的敌机
        if assignment_analysis['multiple_assignment_enemies']:
            logger.info(f"  ⚠️  被多架我方飞机同时攻击的敌机:")
            for enemy_id, info in assignment_analysis['multiple_assignment_enemies'].items():
                logger.info(f"    {enemy_id}: 被 {info['count']} 架飞机攻击 ({', '.join(info['attackers'])})")
        else:
            logger.info(f"  ✅ 所有敌机都被单架我方飞机攻击")
        
        # 计算分配效率
        assignment_efficiency = assignment_analysis['total_enemies_targeted'] / assignment_analysis['total_friendly_aircraft']
        logger.info(f"  分配效率: {assignment_efficiency:.2f} (目标敌机数/我方飞机数)")
        
    except Exception as e:
        logger.error(f"Error logging target assignment analysis: {e}")
...
def update_observation_with_target(obs, env, step_count, center_lon=120.0, center_lat=60.0, center_alt=0.0):
    ...
    try:
        ...
        # 分析目标分配情况
        assignment_analysis = analyze_target_assignment(target_mapping)
        if assignment_analysis:
            log_target_assignment_analysis(assignment_analysis, step_count)
...
def main():
    ...
    experiment_name = "5v4_combat_blood"
    ...
    # 创建5v4空战环境
    logger.info("Creating 5v4 combat environment...")
    try:
        env = MultipleCombatEnv("5v4/NoWeapon/Hierarchy2v1Style")
        env.seed(0)
    ...
    try:
        ...
        ego_policy1v1 = PPOActor(args, env.observation_space, env.action_space, device=device)
        ego_policy2v1 = MAPPOActor(args, env.observation_space, env.action_space, device=device)
    ...
    logger.info("Starting 5v4 combat render...")
    obs, _ = env.reset()
    ...
    # RNN状态初始化 - 5v4配置
    ego_rnn_states = np.zeros((5, 1, 128), dtype=np.float32)  # (5, 1, 128) - 5架红方
    enm_rnn_states = np.zeros((4, 1, 128), dtype=np.float32)  # (4, 1, 128) - 4架蓝方
    ego_masks = np.ones((5, 1))  # 5架红方飞机的掩码
    enm_masks = np.ones((4, 1))  # 4架蓝方飞机的掩码
    
    # 观察数据切片：前5架为红方，后4架为蓝方
    ego_obs = obs[:5, :]  # 红方观察 (5, 15)
    enm_obs = obs[5:, :]  # 蓝方观察 (4, 15)
    
    episode_rewards = np.zeros((5, 1))  # 5架红方飞机
    step_count = 0
    ...
    try:
        while True:
            ...
            if step_count % 50 == 0:  # 每50步打印一次
                ...
                # 打印飞机配置
                logger.info("Aircraft configuration:")
                logger.info("Red team (5 aircraft): A0100, A0200, A0300, A0400, A0500")
                logger.info("Blue team (4 aircraft): B0100, B0200, B0300, B0400")
    ...
    logger.info(f"红方幸存者: {red_survivors}/5, 蓝方幸存者: {blue_survivors}/4")
