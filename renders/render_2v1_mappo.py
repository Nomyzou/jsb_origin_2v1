...
from algorithms.mappo.ppo_actor import PPOActor  # 改为导入MAPPO的actor
import time
...
render = True
ego_policy_index = 866
enm_policy_index = 250
episode_rewards = 0

# 建议切换到 2v1 的权重目录
root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
ego_run_dir = os.path.join(root_dir, "scripts/results/MultipleCombat/2v1/NoWeapon/HierarchySelfplay/mappo/v1_baseline_random_opponents/wandb/latest-run/files/")
...
# 敌方仍然使用PPO的actor（因为敌方模型是1v1训练的）
from algorithms.ppo.ppo_actor import PPOActor as PPOActor_1v1
enm_policy = PPOActor_1v1(args, env.observation_space, env.action_space, device=torch.device("mps"))
ego_policy.eval()
enm_policy.eval()
# Replace manual loading with robust helper
load_actor_weights_with_adaptation(ego_policy, ego_run_dir + f"/actor_{ego_policy_index}.pt", logger_prefix="Ego")
load_actor_weights_with_adaptation(enm_policy, enm_run_dir + f"/actor_{enm_policy_index}.pt", logger_prefix="Enemy")

print("Start render")
obs, _ = env.reset()
if render:
    env.render(mode='txt', filepath=f'{experiment_name}_2v1_mappo.txt.acmi')
